...
/*   Updated: 2022/08/16 18:22:27 by lgiband          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_char_to_img(t_game *game, int x_start, int y_start, t_img_data *img)
{
	...
	while (y < img->height)
	{
		...
		while (x < img->width)
		{
			...
			{
				dst = game->window_image.addr + ((int)(y + y_start) * game->window_image.line_length
						+ (int)(x + x_start) * (game->window_image.bits_per_pixel / 8));
