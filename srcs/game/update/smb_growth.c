...
{
	if (game->player.evolution == LITTLE && (game->player.state == GROWTH || game->player.state == C_GROWTH))
	{
		game->player.all_player_img = game->all_images.all_bplyrs;
		game->player.evolution = BIG;
	}
	else if (game->player.evolution == BIG && (game->player.state == SHRINK || game->player.state == C_SHRINK))
	{
		game->player.all_player_img = game->all_images.all_lplyrs;
		game->player.evolution = LITTLE;
	}
	else if (game->player.evolution == BIG && (game->player.state == GROWTH || game->player.state == C_GROWTH))
	{
		game->player.all_player_img = game->all_images.all_fplyrs;
		game->player.evolution = FIRE;
	}
	else if (game->player.evolution == FIRE && (game->player.state == SHRINK || game->player.state == C_SHRINK))
	{
		game->player.all_player_img = game->all_images.all_bplyrs;
		game->player.evolution = BIG;
	}
