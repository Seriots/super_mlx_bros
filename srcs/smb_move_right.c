...
/*   Updated: 2022/08/08 21:11:03 by lgiband          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	second_part(t_game *game, t_player *player)
{
	if (check_collisions(game, player->x_pos + (game->delay_between_frame * PLAYER_SPEED),
			player->y_pos, game->x_position))
		player->x_pos = game->collide_obj.x - player->width - game->x_position;
	else if (player->x_pos + (game->delay_between_frame * PLAYER_SPEED) + player->width > SCREEN_WIDTH)
...
static int	third_part(t_game *game, t_player *player)
{
	...
	else if (game->x_position + (game->delay_between_frame * PLAYER_SPEED) <= game->map.width - SCREEN_WIDTH)
		game->x_position += (game->delay_between_frame * PLAYER_SPEED);
	else
	{
